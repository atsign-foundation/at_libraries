...
class VerbSyntax {
  ...
  static const sync = r'^sync:(?<from_commit_seq>[0-9]+|-1):(skipDeletes:(?<skipDeletes>true|false):)?(:(?<regex>.+))?$';
  static const syncFrom =
      r'^sync:from:(?<from_commit_seq>[0-9]+|-1)(:limit:(?<limit>\d+)):(skipDeletes:(?<skipDeletes>true|false):)?(:(?<regex>.+))?$';
