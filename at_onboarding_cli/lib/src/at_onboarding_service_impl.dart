...
class AtOnboardingServiceImpl implements AtOnboardingService{
  ...
  AtOnboardingPreference atOnboardingConfig;

  AtClient? get atClient => _atClient;
  AtLookupImpl? get atLookup => _atLookup;

  AtOnboardingServiceImpl(this._atSign, this.atOnboardingConfig);

  ///creates instance of [AtClient] using either of [AtClientPreference] or [AtOnboardingPreference]
  ...
  Future<bool> onboard() async {
    ...
    var secret = atOnboardingConfig.cramSecret ?? getSecretFromQr(atOnboardingConfig.qrCodePath);
  ...
  Future<void> _generateEncryptionKeyPairs() async {
    ...
    //logger.severe('pkam public key ${await _atLookup?.llookup(AT_PKAM_PUBLIC_KEY)}');
    var pubK = await _atLookup?.llookup(AT_PKAM_PUBLIC_KEY);
    pubK = pubK?.split(':')[1];
    //logger.severe('comparing pkam pub key ${pubK == _pkamRsaKeypair.publicKey.toString()}');
    //logger.severe(await _atLookup?.llookup(AT_PKAM_PUBLIC_KEY));
    //logger.severe(_pkamRsaKeypair.privateKey.toString());
